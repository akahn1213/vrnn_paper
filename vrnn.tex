...
To optimally evaluate the model's performance, the weights corresponding to a training period of 100 epochs were chosen in the following studies. Figure \ref{fig:score_transform} shows the distributions of the Anomaly Score for leading jets in both the background sample and the two-prong signal sample after applying the transformation in equation 18.

\begin{figure}[H]
	\begin{center}
		\includegraphics[width=250pt]{imgs/bugfix/2Prong_Contaminated_10p0_J_Score_Multi_Lead_SaveForPaper.png}
	\end{center}
	\caption{Anomaly Score after applying the transformation described in equation 18. The Anomaly Score in this figure is computed from the leading jets of each event, for both the background sample and two-prong signal sample.}
