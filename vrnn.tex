...
\begin{figure}
  \begin{center}
    ...
    \begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\layersep]
        ...
        \node[draw, rectangle, very thick, minimum width=1cm, minimum height=2cm, label=$\boldsymbol{\mu}$] (r1) at (4cm, -2.5cm) {};
        \node[draw, rectangle, very thick, minimum width=1cm, minimum height=2cm, label=below:$\boldsymbol{\sigma}$] (r1) at (4cm, -4.5cm) {};
        \node[draw, rectangle, very thick, minimum width=1cm, minimum height=2cm, label=below:{$\textbf{z}=\boldsymbol{\mu} + \boldsymbol{\sigma} \epsilon$}] (r1) at (6cm, -3.5cm) {};

        \draw [decorate,decoration={brace,amplitude=10pt, mirror},xshift=-4pt,yshift=0pt] (-0.25cm,-0.75cm) -- (-0.25cm,-6.25cm) node [black,midway,xshift=-0.6cm] {\footnotesize $\textbf{x}$}; 
        \draw [decorate,decoration={brace,amplitude=10pt},xshift=-4pt,yshift=0pt] (10.5cm,-0.75cm) -- (10.5cm,-6.25cm) node [black,midway,xshift=0.6cm] {\footnotesize $\textbf{y}$}; 
...
\end{equation} 
Performing this particular step is the primary function of traditional RNN architectures)~\cite{lstm, cho2014learning}. %so any such architecture can be conveniently chosen to perform this task.
...
\SetKw{Beginning}{Start}
  ...
  \eIf{Any constituents have $\Delta R > 1$\footnote{$\Delta R$ is computed as $\sqrt{\eta^2 + \phi^2}$ for each constituent, where $\eta$ and $\phi$ are measured relative to the x axis.} }{
...
\begin{figure}[H]
	\begin{center}
		\includegraphics[width=213pt]{imgs/3Prong_Contaminated_10p0_J_Mass_Multi_Lead_SaveForPaper.png}
		\includegraphics[width=213pt]{imgs/3Prong_Contaminated_10p0_J_Mass_AnomScore0p65_Multi_Lead_SaveForPaper.png}
	\end{center}
